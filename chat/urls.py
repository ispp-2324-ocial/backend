...
urlpatterns = [
    ...
    path('<int:chat_id>/createMessage/', views.MessageCreate.as_view(), name='message_create'),
    path('<int:pk>/delete/', views.ChatDelete.as_view(), name='chat_delete'),
    path('<int:pk>/deleteMessage/', views.MessageDelete.as_view(), name='message_delete'),
